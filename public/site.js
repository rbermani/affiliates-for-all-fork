...
function Details(script, tab, validator) {
    ...
    $("#details_cancel").click(function() {
        $("#tabs").tabs("select", obj.tab - 1).tabs("disable", obj.tab);
    });
    ...
    if(this.validator()) {
        ...
        getJSON(this.script, result, function(json) {
            if(json === true) {
                ...
                $("#saved").dialog({ autoOpen: false,
                                 buttons: { "Ok": function() { $(this).dialog("close");
                                     $("#tabs").tabs("select", tab).tabs("disable", obj.tab);} } });
...
Details.prototype.addNew = function() {
    this.key = null;
    $("#tabs").tabs("enable", this.tab).tabs("select", this.tab);
